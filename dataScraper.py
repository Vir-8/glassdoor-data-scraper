...
for i, position in enumerate(job_positions):
    url = f"{base_url}{position.lower().replace(' ', '-')}-jobs-SRCH_IL.0,5_IN115_KO0,{len(position)}.htm"
    ...
    if response.status_code == 200:
        ...
        if salary_elements:
            ...
            for salary_element in salary_elements:
                ...
                position_data[position].append({"salary": salary, "location": location})
                print(salary, location)
            print()
            time.sleep(2)
        else:
...
with open(csv_filename, mode="w", newline="", encoding="utf-8") as file:
    ...
    # Write header
    header = ["Position", "Location", "Salary"]
    ...
    for data in filtered_data:
        ...
        # Write data to CSV
        data_row = [position, location, salary]
